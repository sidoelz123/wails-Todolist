<template>
  <div v-for="item in items" :key="item.id" class="group relative -mr-4"
    ...
    <!-- Avatar Image -->
    <component v-if="isLucideIcon(item.image)" :is="getLucideIconComponent(item.image)"
      class="size-8 transition duration-500 group-hover:z-30 group-hover:scale-105" />
    <img v-else :src="item.image" :alt="item.name"
      class="relative !m-0 size-14 border-2 rounded-full border-white object-cover object-top !p-0 transition duration-500 group-hover:z-30 group-hover:scale-105" />
  </div>
</template>
